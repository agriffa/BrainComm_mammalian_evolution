function varargout=notBoxPlot_dotcolor(y,x,jitter,style,dotcolor)
...
for k=1:length(X)
    ...
    h(k).data=plot(thisX+J, thisY, 'o', 'color', dotcolor,... % COLOR OF DATA POINTS
                   'markerfacecolor', dotcolor,'markersize',7);        
end
...
if strcmp(style,'line') | strcmp(style,'sdline')
  for k=1:length(X)    
    ...
    h(k).sd=plot([X(k),X(k)],[mu(k)-SD(k),mu(k)+SD(k)],...
                 '-','color',[84, 153, 199]./255,'linewidth',4);
...
if strcmp(style,'line')
    for k=1:length(X)     
        %Plot mean and SEM
        h(k).mu=plot(X(k),mu(k),'o','color',[31, 97, 141]./255,...
            'markerfacecolor',[31, 97, 141]./255,...
            'markersize',12);
        
        h(k).sem=plot([X(k),X(k)],[mu(k)-SEM(k),mu(k)+SEM(k)],'-','color',[84, 153, 199]./255,...
            'linewidth',3);   
