function varargout=notBoxPlot_modified_paired(y,x,jitter,style)
...
% If x or y has not even dimension, return an error
if rem(length(x),2) > 0 || rem(size(y,2),2) > 0 
    disp('Error - Input must have an even number of columns');
    return
end

if nargin<3 || isempty(jitter)
    jitter=0; %larger value means greater amplitude jitter
    jitterbox=0.3;
end
...
hold on % this also create a new figure
[uX,a,b]=unique(x);

h1=[];
h2=[];
h3=[];
% Loop over boxplots than need to be drawn
for ii=1:length(uX)
    f=find(b==ii);
    h1=[h1,myPlotter(x(f),y(:,f))];
    if rem(ii,2) == 0
        h2=[h2,myLines(x([f-1,f]),y(:,[f-1,f]),f)];
        h3=[h3,myPoints(x(f-1),y(:,f-1))]; 
    end
    h3=[h3,myPoints(x(f),y(:,f))];
end
...
cols(1,:)=0;
jitScale=jitterbox*0.55; %To scale the patch by the width of the jitter
...
for k=1:length(X)
    ...
    if strcmp(style,'patch') 
      %h(k).sdPtch=patchMaker(SD(k),[0.6,0.6,1]); % COLOR OF THE 95 PERC
      h(k).sdPtch=patchMaker(SD(k),[0.8784,0.8784,0.8784]);
      %h(k).sdPtch=patchMaker(SD(k),[0.6,0.898,1]);
    end
    
    if strcmp(style,'patch') || strcmp(style,'sdline')
      %h(k).semPtch=patchMaker(SEM(k),[1,0.6,0.6]); % COLOR OF THE +- STD
      h(k).semPtch=patchMaker(SEM(k),[0.8,0.898,1]); % COLOR OF THE +- STD
      h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],'-k',...
           'linewidth',1.5); % COLOR OF THE MEAN LINE
    end
    ...
    h(k).data=plot(thisX+J, thisY, 'o', 'color', C,... % COLOR OF DATA POINTS
...
function h=myLines(X,Y,kk)
    ...
    for k = 1:size(Y,1)
        ...
        thisy1 = Y(k,1);
        thisy2 = Y(k,2);
        h(kk).line=plot([thisx1 thisx2], [thisy1 thisy2], '-', 'color', [144, 148, 151]./255);
    end
end % function myLines


function h=myPoints(X,Y)
    %The plot colors to use for multiple sets of points on the same x
    %location
    cols=hsv(length(X)+1)*0.5;
    cols(1,:)=0;
